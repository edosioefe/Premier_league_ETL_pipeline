...
def get_top_assisters_json(url, ta_url, api_key, season, leagueid, requests, sys, json):
    ...
    try:
        res = requests.get(ta_url, params=query_param, headers=header_param)
...
# transform data
def top_assisters_transformation(json_data,pd):
        ...
        df = pd.concat([df[['player.name','player.firstname','player.lastname']],
               stat_df[['team.name','games.appearences','goals.assists']] ],axis=1)
        df = df.head()
        return df 



def top_assisters_to_staging(string, df, df_type, create_engine):
    try:
